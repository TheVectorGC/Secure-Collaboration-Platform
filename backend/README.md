...
- `local` — локальная разработка, Swagger включён, actuator открыт шире, PostgreSQL доступен с `postgres/postgres`;
...
Локальный root compose запускает сервисы с профилем `local`.
...
При запуске из IDEA профиль по умолчанию — `local`.

## Swagger

В `local` профиле Swagger доступен по адресам:
...
В `local` профиле доступны расширенные actuator endpoints, включая metrics и loggers.
...
Redis используется в `realtime-gateway` для presence:
...
Request id добавляется в MDC и попадает в логи, чтобы можно было связать цепочку запросов между сервисами.
...
В `local` профиле bootstrap admin включается через env:
