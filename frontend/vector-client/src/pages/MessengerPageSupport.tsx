...
export function DocumentCreationModal({
  ...
  const signerOptions = useMemo(() => getActiveGroupParticipantAccountIds(selectedChat)
    .filter((accountId) => accountId !== currentAccountId || selectedChat?.type === 'SELF'), [currentAccountId, selectedChat]);
    ...
    <div className="absolute inset-0 z-40 flex items-center justify-center bg-black/55 p-4 backdrop-blur-md">
      <div className="flex max-h-[88vh] w-full max-w-3xl flex-col overflow-hidden rounded-[2rem] border border-white/10 bg-[#151721] shadow-2xl shadow-black/60">
        <div className="border-b border-white/10 p-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <div className="mt-1 text-sm text-zinc-500">Настройте карточку документа и выберите сотрудников, от которых нужна подпись.</div>
            </div>
          ...
          <div className="mt-5 rounded-[1.5rem] border border-white/10 bg-white/[0.035] p-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <div className="mt-1 text-xs text-zinc-500">Подписывать и отклонять смогут только выбранные сотрудники.</div>
              </div>
              <div className="rounded-full bg-white/[0.06] px-3 py-1 text-xs text-zinc-400">{selectedSignerAccountIds.length}/{signerOptions.length}</div>
            </div>
            ...
            <div className="mt-4 grid gap-2 sm:grid-cols-2">
              {signerOptions.map((accountId) => {
                ...
                return (
                  ...
                  >
                    <UserAvatar label={getAccountDisplayName(accountId, profilesById)} imageUrl={getAccountAvatarUrl(profile)} size="sm" />
                    <span className="min-w-0 flex-1">
                      <span className="block truncate text-sm font-medium text-zinc-100">{getAccountDisplayName(accountId, profilesById)}</span>
                      <span className="block truncate text-xs text-zinc-500">{getAccountUsernameLabel(accountId, profilesById)}</span>
                    </span>
        ...
        <div className="flex items-center justify-end gap-3 border-t border-white/10 p-5">
          ...
          <button
            onClick={() => void onConfirm({ file, title, description, requiredSignerAccountIds: selectedSignerAccountIds })}
          ...
          {isLoading ? (
            ...
            <div className="space-y-4">
              {documents.map((documentItem) => {
                ...
                const canCancel = Boolean(activeAccountId && activeAccountId === documentItem.ownerAccountId && !['FULLY_SIGNED', 'CANCELLED'].includes(documentItem.status));
                          ...
                          <div key={signer.signerId} className="rounded-2xl border border-white/10 bg-black/12 p-3">
                            <div className="flex items-center gap-3">
                              ...
                              <div className={`rounded-full px-2 py-1 text-[11px] ${signer.status === 'SIGNED' ? 'bg-emerald-500/15 text-emerald-200' : signer.status === 'REJECTED' ? 'bg-red-500/15 text-red-200' : 'bg-amber-500/15 text-amber-100'}`}>
                                {signer.status === 'SIGNED' ? 'OK' : signer.status === 'REJECTED' ? 'Отказ' : 'Ждём'}
                              </div>
                            </div>
