...
async function prepareBackupUnlockKey(accountId: string, accountPassword: string) {
  if (!window.vectorCrypto) {
    return;
  }

  try {
    const existingAccountBackupProfile = await getCurrentAccountBackupProfile();
    await window.vectorCrypto.setAccountBackupPassword({
      accountId,
      password: accountPassword,
      kdfSaltBase64: existingAccountBackupProfile.kdfSaltBase64,
      kdfParametersJson: existingAccountBackupProfile.kdfParametersJson,
    });
    return;
  }
  catch (error) {
    if (!isNotFoundError(error)) {
      console.warn('Unable to prepare backup unlock key from existing account backup profile.', error);
    }
  }
